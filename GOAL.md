...
Build a fun, calming, local-first personal panic-support app that helps someone at pre-panic or active-panic moments move through simple, safe, evidence-informed coping steps with as little cognitive load as possible.

## Primary Objectives

| Objective | Why | Success Signal |
| --- | --- | --- |
| Immediate SOS support | A panicking user cannot navigate menus or dense copy. | From the first screen, the user can start guided support in one obvious tap and complete a calming flow without network access. |
| Pre-panic practice and prevention | The app is more useful before panic fully escalates. | The app includes practice tools for breathing, grounding, reassurance phrases, sensory toolkit prep, and light mood-shifting activities. |
| Personalization without accounts | Panic support works better when the words, contact, and tools feel personal. | Reassurance phrases, support contact, toolkit items, low-stimulation preference, and journal entries persist locally on the device. |
...
| Fun and helpful tone | A panic app should not feel clinical, punitive, or boring. | It includes gentle playful moments such as Spark Joy prompts, comforting micro-rewards, and optional character/companion ideas without blocking the SOS path. |
...
The target user is someone with panic disorder who may be scared, overstimulated, embarrassed, or racing cognitively. The app should give them a friendly place to land: one-tap grounding, paced breathing, reassuring language, sensory distraction ideas, quick access to a trusted person, and a post-episode note without judgment.
...
- education panels about panic and panic toolkit prep,
...
Do not make the initial app depend on webcam, microphone, WebSockets, Anthropic, ElevenLabs, MediaPipe, Three.js, or server infrastructure. Those are optional future experiments after the panic-support MVP is stable.

## In Scope

- Create the main panic-support app from `prefile.md` as a first-party app in this project.
- Use React, TypeScript, Vite, and a local bundled dependency workflow.
- Build mobile-first responsive screens for active panic and pre-panic use.
...
- The safest default is a local-only, no-login app with no network requirement for panic support.
...
- Optimizing for desktop before mobile panic use cases are handled.
